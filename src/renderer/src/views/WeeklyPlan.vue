...
# WeeklyPlan View
## Behavior
- Display header "Weekly Plan"
- Display week selector (1-12)
  - Highlight current week
  - Clicking a week updates the view
- Display list of Projects using ProjectCard (filtered by selected week context if applicable, or just projects to track weekly progress)
- Button to "Add New Project" (maybe less relevant here if this is just for planning, but keeping for now as it was in the original structure the user liked)
  - Opens a modal or form to create a project
  - Fields: Title, Description, Deadline (Optional, defaults to 12 weeks from now or end of quarter)
...
const currentMonth = computed(() => Math.ceil(store.currentWeek / 4))
...
<template>
  <div class="h-full flex flex-col p-8 overflow-hidden">
    ...
    <div class="flex justify-between items-center mb-8">
      <div>
        <h1 class="text-3xl font-bold text-morandi-blue-light mb-2">Weekly Plan</h1>
        <p class="text-gray-400">Q{{ store.currentQuarter }} {{ store.currentYear }} - Month {{ currentMonth }} - Week {{ store.currentWeek }}</p>
      </div>
      <button 
    ...
    <!-- Week Selector -->
    <div class="flex space-x-2 mb-8 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent">
      <button
        v-for="week in weeks"
        :key="week"
        @click="store.currentWeek = week"
        class="px-4 py-2 rounded-full text-sm font-medium transition-all whitespace-nowrap"
        :class="store.currentWeek === week 
          ? 'bg-morandi-blue text-gray-900 shadow-lg shadow-morandi-blue/20' 
    ...
    <div class="flex-grow overflow-y-auto pr-2 -mr-2">
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-8">
        <ProjectCard
